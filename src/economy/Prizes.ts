import { MYSTERY_GAME_TICKETS } from './pricing';

export const PRIZES_STORAGE_KEY = 'bibliothek.prizes.v1';
...
export const PRIZES: readonly Prize[] = [
  { id: 'keyring', name: 'Joystick keyring', kind: 'keyring', tickets: 30, color: 0xd23a3a, blurb: 'A tiny red-ball joystick on a ring.' },
  { id: 'ball', name: 'Bouncy ball', kind: 'ball', tickets: 40, color: 0x33e0ff, blurb: 'Glitter inside. Bounces higher than it should.' },
  { id: 'yoyo', name: 'Neon yo-yo', kind: 'yoyo', tickets: 50, color: 0x4dff7a, blurb: 'Glows a little in the dark.' },
  { id: 'duck', name: 'Rubber duck', kind: 'duck', tickets: 60, color: 0xffd23a, blurb: 'Squeaks. Wears sunglasses.' },
  { id: 'bear', name: 'Plush bear', kind: 'bear', tickets: 150, color: 0xb07a4a, blurb: 'The one from the glass case.' },
  { id: 'cat', name: 'Plush cat', kind: 'cat', tickets: 150, color: 0x8a8f99, blurb: 'Grey, smug, a lot like someone at home.' },
  { id: 'rocket', name: 'Tin rocket', kind: 'rocket', tickets: 250, color: 0xc8443a, blurb: 'Wind-up. The key is stiff.' },
  { id: 'lavaLamp', name: 'Lava lamp', kind: 'lavaLamp', tickets: 350, color: 0xff2fa0, blurb: 'Magenta wax, takes an hour to get going.' },
  { id: 'trophy', name: 'Champion trophy', kind: 'trophy', tickets: 400, color: 0xd4a52a, blurb: 'Gold-effect plastic. ARCADE CHAMPION on the base.' },
  { id: 'miniCabinet', name: 'Mini cabinet', kind: 'miniCabinet', tickets: 600, color: 0x2f4f8f, blurb: 'A cabinet the size of a shoebox. The screen lights up.' },
  { id: 'catToy', name: 'Feather wand', kind: 'catToy', tickets: 80, color: 0x33e0ff, blurb: 'For someone at home. It lies by the armchairs; watch who comes to play.', home: 'catToy' },
  { id: 'poster', name: 'Arcade poster', kind: 'poster', tickets: 120, color: 0xff2fa0, blurb: 'NEON SHERIFF, rolled up. It goes on the bedroom wall.', home: 'poster' },
  { id: 'moodLamp', name: 'Neon mood lamp', kind: 'moodLamp', tickets: 300, color: 0xb05cff, blurb: 'Stands on the bedroom dresser. Click it to change the colour.', home: 'moodLamp' },
  { id: 'mysteryGame', name: 'Mystery game', kind: 'mystery', tickets: MYSTERY_GAME_TICKETS, color: 0xffd23a, blurb: 'A wrapped cartridge: a random game for the collection. It comes home in the parcel.', game: true },
...
export class PrizeStore {
  ...
  constructor(
    ...
    private readonly key = PRIZES_STORAGE_KEY,
  ) {
    this.list = this.load();
  }
  ...
  add(id: string): void {
    ...
    try {
      ...
      console.warn('[prizes] could not persist', err);
    }
  ...
  private load(): OwnedPrize[] {
    try {
      const parsed = JSON.parse(this.storage?.getItem(this.key) ?? 'null') as unknown;
      if (!Array.isArray(parsed)) return [];
      return parsed.filter((p): p is OwnedPrize => typeof p === 'object' && p !== null && typeof p.id === 'string' && !!getPrize(p.id)).map((p) => ({ id: p.id, at: typeof p.at === 'string' ? p.at : '' }));
    } catch {
      return [];
    }
  }
