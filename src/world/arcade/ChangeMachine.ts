...
export interface ChangeMachineOptions {
  /** What clicking it gets the player told, one line at a time. Default: it is out of order. */
...
const DEFAULT_LINES = ['OUT OF ORDER. Tickets turn into coins at the prize counter.', 'It ate a note in 2009 and never gave change again.'];
 ...
 * header, a note acceptor, a coin slot and a return cup, and a hand-written OUT OF ORDER note
 * taped over the slot. Clicking it gets a line (`SessionActions.hint`). Origin on the floor under
...
export class ChangeMachine extends THREE.Group implements Furniture, Interactable {
  ...
  constructor(options: ChangeMachineOptions = {}) {
    ...
    const panel = new THREE.Mesh(new THREE.PlaneGeometry(WIDTH - 0.04, panelH), new THREE.MeshStandardMaterial({ map: paintPanel(WIDTH - 0.04, panelH), roughness: 0.5, metalness: 0.2 }));
...
function paintPanel(wM: number, hM: number): THREE.Texture {
  ...
  // The taped note, askew over the slot: torn paper, marker capitals, two strips of tape.
  ctx.save();
