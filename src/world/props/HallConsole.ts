...
export interface HallConsoleOptions {
  ...
  width?: number;
  /** A mirror hung on the wall above it. Default true. */
  mirror?: boolean;
}
...
/**
 * A narrow walnut console against a wall: a bowl with the keys in it, the mail in a small pile, a
 * plant at the end, and a mirror over it. Wall-hung with `y: 0`: origin on the floor at the wall,
...
export class HallConsole extends THREE.Group implements Furniture {
  ...
  constructor(options: HallConsoleOptions = {}) {
    ...
    this.add(bowl);
    part(this, 0.03, 0.008, 0.06, BRASS, { x: -width * 0.22 - 0.02, y: TOP_Y + 0.054, z: z + 0.01 });
